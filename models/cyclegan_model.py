...
class CycleGanmodel(BaseModel):
    ...
    def set_input(self, input):
        ...
        self.depth = self.depth[:,0,:,:].unsqueeze(1) # Convert depth image to grayscale
        self.real_B = input['C' if AtoB else 'A'].to(self.device)
        self.real_B_depth = input['E'].to(self.device)
        self.real_B_depth = self.real_B_depth[:,0,:,:].unsqueeze(1) # Convert Real depth image to grayscale
